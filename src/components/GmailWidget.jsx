    ...
    try {
      ...
      if (response.data.success) {
        ...
        showMessage(
          "success",
          `Fetched ${
            (response.data.data.emails || response.data.emails || []).length
          } emails successfully`
        );
      } else {
    ...
    return (
      <div className="space-y-1 max-h-[320px] overflow-y-auto custom-scrollbar">
        {emails.slice(0, 10).map((email, index) => {
          ...
          let emailDate =
            email.date || email.receivedDateTime || email.internalDate;
          ...
          const emailData = {
            ...
            hasAttachment:
              email.hasAttachment ||
              email.payload?.parts?.some((part) => part.filename) ||
              false,
          };
                ...
                <div className="flex items-center justify-between mb-1">
                  <span
                    className={`text-sm font-medium truncate ${
                      emailData.unread ? "text-white" : "text-gray-300"
                    }`}
                  >
  ...
  const renderExternalView = () => (
    <div className="text-center py-8">
      ...
      <p className="text-gray-400 mb-6 max-w-sm mx-auto">
        {error ||
          "Gmail integration is not available. Access your emails directly through Gmail."}
      </p>
